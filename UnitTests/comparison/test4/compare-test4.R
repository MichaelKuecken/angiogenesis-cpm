...
inJSON <- read_json(  "test4-comparison.json" )
...
pComp <- ggplot( dall, aes( x = time, y = conc, color = implementation ) ) + 
	...
	labs( x = "time (MCS)", y = "c(100,100)", color = NULL ) +
	mytheme + theme(
...
pComp2 <- ggplot( dall, aes( x = time, y = delta, color = implementation2 ) ) + 
	geom_line() + 
	scale_color_manual( values = cmap ) + 
	labs( x = "time (MCS)", y = "c(100,100) (fold-change wrt REF)", color = NULL ) +
	mytheme + theme(
...
ggsave( p, file = "test4-comparison.pdf", width = 15, height = 7, units="cm", useDingbats = FALSE )
